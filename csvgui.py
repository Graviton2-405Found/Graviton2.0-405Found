...
def generate_csv():
    with open('C:/Users/laksh/OneDrive/Documents/Desktop/checkout.csv', 'w', newline='') as csvfile:
        fieldnames = ['Product', 'Price']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

        writer.writeheader()
        for i in range(product_listbox.size()):
            product_info = product_listbox.get(i).split(' - Rs. ')
            writer.writerow({'Product': product_info[0], 'Price': product_info[1]})
        writer.writerow({'Product': 'Total', 'Price': total_cost})
...
def start_http_server():
    ...
    # After the server starts, open the URL in the default web browser
    download_url = 'http://localhost:8000/checkout.csv'
...
checkout_button = tk.Button(window, text="Checkout", state=tk.DISABLED, bg=button_bg_color, fg=button_fg_color, font=label_font, command=generate_csv)
